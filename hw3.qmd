...
``` {r}
mean_red <- df %>% filter(type == "red") %>% summarise(mean_red_quality = mean(quality, na.rm = TRUE))
mean_white <- df %>% filter(type == "white") %>% summarise(mean_white_quality = mean(quality, na.rm = TRUE))
diff_mean <- mean_white$mean_white_quality - mean_red$mean_red_quality
```
```{r}
